...
class MetricsGenerator:
    ...
    def _save_metric_data(self, match, data, metric, header, samples_per_batch):
        ...
        np.savetxt(file_name, data, delimiter=",", header=header, comments="", fmt="%.4f")
        return file_name
    ...
    def compute_motion_feature_metrics(self, mse_metric=False, bhatt_metrics=False):
        ...
        if mse_metric:
            mfeat_mse = self._motion_feature_by_mse(mf_2D_pred, mf_2D_gt, mf_1D_pred, mf_1D_gt)
        if bhatt_metrics:
            mfeat_bhatt_dist, mfeat_bhatt_coef = self._motion_feature_by_bhattacharyya(mf_2D_pred, mf_2D_gt, mf_1D_pred, mf_1D_gt)
        ...
        for i in range(nsamples):
            ...
            for j in range(pred_len):
                for c in range(self.params.MPROPS_COUNT):
                    ...
                    nsamples_tv_otime[i, j* self.params.MPROPS_COUNT + c] = np.abs(tv_pred - tv_gt)
