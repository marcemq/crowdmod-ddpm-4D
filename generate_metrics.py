...
def save_all_boxplots_metrics(metrics_data_dict, metrics_header_dict, title):
    ...
    if len(metrics_df_dict['MIN-ENERGY']) != 0:
        merge_and_plot_boxplot(df_max=metrics_df_dict['MIN-ENERGY'], df=metrics_df_dict['ENERGY'], title=f"ENERGY and MIN-ENERGY of {title}", save_path=f"{cfg.MODEL.OUTPUT_DIR}/BP_ENERGY.png", ytick_step=2, prefix='min-')
